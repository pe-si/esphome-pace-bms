...
bool PaceBmsProtocolV20::ProcessReadAnalogInformationResponse(const uint8_t busId, OPTIONAL_NS::optional<uint8_t> respondingBusId, const std::vector<uint8_t>& response, AnalogInformation& analogInformation)
{
	...
	{
		return ProcessReadAnalogInformationResponse_PYLON(busId, respondingBusId, response, analogInformation);
	}
	else if (variant_to_use == "SEPLOS")
	{
		return ProcessReadAnalogInformationResponse_SEPLOS(busId, respondingBusId, response, analogInformation);
	}
	else if (variant_to_use == "EG4")
	{
		return ProcessReadAnalogInformationResponse_EG4(busId, respondingBusId, response, analogInformation);
	}
	else
	{
...
bool PaceBmsProtocolV20::ProcessReadStatusInformationResponse(const uint8_t busId, OPTIONAL_NS::optional<uint8_t> respondingBusId, const std::vector<uint8_t>& response, StatusInformation& statusInformation)
{
	...
	if (variant_to_use == "PYLON")
	{
		return ProcessReadStatusInformationResponse_PYLON(busId, respondingBusId, response, statusInformation);
	}
	else if (variant_to_use == "SEPLOS")
	{
		return ProcessReadStatusInformationResponse_SEPLOS(busId, respondingBusId, response, statusInformation);
	}
	else if (variant_to_use == "EG4")
	{
		return ProcessReadStatusInformationResponse_EG4(busId, respondingBusId, response, statusInformation);
	}
	else
	{
		LogError("Invalid protocol variant '" + variant_to_use + "'");
